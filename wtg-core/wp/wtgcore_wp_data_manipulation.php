<?php
# RENAME THIS FILE TO yts_strings
/**
...
*/
function yts_STRINGS_prepend_exists($haystack,$needle){
...
*/
function yts_STRINGS_strtotime($date_string,$format = 'UK'){
    if($format == 'US'){
...
*/
function yts_STRINGS_is_alphanumeric($string,$minimum_length = 1,$maximum_length = 99){
...
* @return string
*/
function yts_encoding_clean_string($content) {
    global $yts_wtgcore_o42chars;

    if ( strtoupper( get_option('blog_charset' )) == 'UTF-8') {
        $content = str_replace($yts_wtgcore_o42chars['utf8'], $yts_wtgcore_o42chars['feed'], $content);
    }
    
    $content = str_replace($yts_wtgcore_o42chars['ecto'], $yts_wtgcore_o42chars['feed'], $content);
    $content = str_replace($yts_wtgcore_o42chars['in'], $yts_wtgcore_o42chars['feed'], $content);

    return $content;
}  

// Character Encoding Input Array
$yts_wtgcore_o42chars['in'] = array(
    chr(196), chr(228), chr(214), chr(246), chr(220), chr(252), chr(223)
);
$yts_wtgcore_o42chars['ecto'] = array(
    '�', '�', '�', '�', '�', '�', '�'
);
$yts_wtgcore_o42chars['html'] = array(
    '&Auml;', '&auml;', '&ouml;', '&ouml;', '&Uuml;', '&uuml;', '&szlig;'
);
$yts_wtgcore_o42chars['utf8'] = array(
    utf8_encode('�'), utf8_encode('�'), utf8_encode('�'), utf8_encode('�'),
    utf8_encode('�'), utf8_encode('�'), utf8_encode('�')
);
$yts_wtgcore_o42chars['perma'] = array(
    'Ae', 'ae', 'Oe', 'oe', 'Ue', 'ue', 'ss'
);

// Character Encoding Ouput Array
$yts_wtgcore_o42chars['post'] = array(
    '�', '�', '�', '�', 'Uuml;', '�', '�'
);
$yts_wtgcore_o42chars['feed'] = array(
    '&#196;', '&#228;', '&#214;', '&#246;', '&#220;', '&#252;', '&#223;'
);
...
* @return string
*/
function yts_encoding_clean_permalinks($title){
    global $yts_wtgcore_o42chars;
    ...
    $title = str_replace($yts_wtgcore_o42chars['ecto'], $yts_wtgcore_o42chars['perma'], $title);
    $title = str_replace($yts_wtgcore_o42chars['in'], $yts_wtgcore_o42chars['perma'], $title);
    $title = str_replace($yts_wtgcore_o42chars['html'], $yts_wtgcore_o42chars['perma'], $title);
    $title = sanitize_title_with_dashes($title);
    return $title;
}

function yts_remove_last_comma($s){
