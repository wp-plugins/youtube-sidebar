...
* @deprecated, use yts_n()
*/
function yts_notice($message,$type = 'success',$size = 'Extra',$title = false, $helpurl = 'www.youtubesidebar.com/support', $output_type = 'echo',$persistent = false,$clickable = false,$user_type = false){
    ...
    if(is_admin() || $output_type == 'public'){
        ...
        if($output_type == 'return' || $output_type == 'public'){   
            return yts_notice_display($type,$helpurl,$size,$title,$message,$clickable,$persistent);
        }else{
            global $yts_notice_array;

            // establish next array key
            $next_key = 0;
            if(isset($yts_notice_array['notifications'])){
                $next_key = yts_get_array_nextkey($yts_notice_array['notifications']);
            }
            
            // add new message to the notifications array
            $yts_notice_array['notifications'][$next_key]['message'] = $message;
            $yts_notice_array['notifications'][$next_key]['type'] = $type;
            $yts_notice_array['notifications'][$next_key]['size'] = $size;
            $yts_notice_array['notifications'][$next_key]['title'] = $title;
            $yts_notice_array['notifications'][$next_key]['helpurl'] = $helpurl; 
            $yts_notice_array['notifications'][$next_key]['clickable'] = $clickable; 
            
            yts_update_option_persistentnotifications_array($yts_notice_array);       
        }
    }
}

/**
* New Notice Function (replacing yts_notice())
...
*/
function yts_n($title,$mes,$style,$size,$atts = array()){
    ...
    // this allows the notice to be printed within content where the function is called rather than within the $yts_notice_array loop
    if($output == 'return' || $output == 'public'){
        return yts_notice_display($style,$url,$size,$title,$mes,$clickable,$persistent = false);
    }
    
    // arriving here means normal, most common output to the backend of Wordpress
    $yts_notice_array = yts_get_option_persistentnotifications_array();
    ...
    $yts_notice_array['notifications'][$next_key]['message'] = $mes;
    $yts_notice_array['notifications'][$next_key]['type'] = $style;
    $yts_notice_array['notifications'][$next_key]['size'] = $size;
    $yts_notice_array['notifications'][$next_key]['title'] = $title;
    $yts_notice_array['notifications'][$next_key]['helpurl'] = $url; 
    $yts_notice_array['notifications'][$next_key]['output'] = $output;
    $yts_notice_array['notifications'][$next_key]['audience'] = $audience;
    $yts_notice_array['notifications'][$next_key]['user_mes'] = $user_mes;
    $yts_notice_array['notifications'][$next_key]['side'] = $side;
    $yts_notice_array['notifications'][$next_key]['clickable'] = $clickable;
...
*/
function yts_persistentnotice_output($placement_type,$placement_specific = false,$pageid = false){
    $yts_notice_array = yts_get_option_persistentnotifications_array();

    if(!is_array($yts_notice_array) || !isset($yts_notice_array['notifications'])){
        return false;
    }   

    foreach($yts_notice_array['notifications'] as $key => $n){
        if(isset($n['persistent']) && $n['persistent'] == true){
            if($placement_type == $n['placement_type'] && $placement_specific == $n['placement_specific'] && $pageid == $n['pageid']){
                echo yts_persistentnotice_display($n['type'],$n['helpurl'],$n['size'],$n['title'],$n['message'],true,$n['id']);
            }
        }
    }
}
...
* @param mixed $atts
*/
function yts_notice_persistent($title,$message,$type,$size,$atts){
    ...
    if(isset($per['notifications'])){
        $next_key = yts_get_array_nextkey($per['notifications']);
    }   
    ...
    $yts_notice_array['notifications'][$next_key]['side'] = $side;
    $yts_notice_array['notifications'][$next_key]['clickable'] = $clickable;
    $yts_notice_array['notifications'][$next_key]['placement_type'] = $placement_type;      
    $yts_notice_array['notifications'][$next_key]['placement_specific'] = $placement_specific;  
    $yts_notice_array['notifications'][$next_key]['pageid'] = $pageid;  
    $yts_notice_array['notifications'][$next_key]['persistent'] = true;
                        
    // generate a unique notice ID, no need to validate it if we use time()
    $yts_notice_array['notifications'][$next_key]['id'] = $id;

    yts_update_option_persistentnotifications_array($yts_notice_array);
}

function yts_persistentnotice_display($type,$helpurl,$size,$title,$message,$persistent = false,$id = false){
    // ID is required to prevent conflict with other persistent notices and for deleting them
    if($id == false){
        return;
    }
    
    global $yts_form_action; 
    ...
    <form id="yts_notice_delete" method="post" name="yts_notice_delete" action="'.$yts_form_action.'">
        
        <input type="hidden" id="yts_post_processing_required" name="yts_post_processing_required" value="true">       
        <input type="hidden" id="yts_post_deletenotice_id" name="yts_post_deletenotice_id" value="'.$id.'">       
        <input type="hidden" id="yts_post_deletenotice" name="yts_post_deletenotice" value="true">
        
        <div class="jquerybutton">
        
            <button id="yts_notice_delete_'.$id.'">Delete Notice</button>
...
/**
* Outputs the contents of $yts_notice_array, used in yts_header_page.
* Will hold new and none persistent notifications. May also hold persistent. 
*/
function yts_notice_output(){
    $yts_notice_array = yts_get_option_persistentnotifications_array();
    if(isset($yts_notice_array['notifications'])){
       
        foreach($yts_notice_array['notifications'] as $key => $notice){
            ...
            if(!isset($notice['persistent']) || $notice['persistent'] != false){
                ...
                echo yts_notice_display($notice['type'],$notice['helpurl'],$notice['size'],$notice['title'],$notice['message'],$notice['clickable'],$notice['persistent'],$notice['id']);                                               
            
                // notice has been displayed so we now removed it
                unset($yts_notice_array['notifications'][$key]);
            }
        }
...
*/
function yts_notice_display($type,$helpurl,$size,$title,$message,$clickable,$persistent = false,$id = false){
...
* 
* @deprecated use yts_n_postresult as of 13th February 2013
*/
function yts_notice_postresult($type,$title,$message,$helpurl = false,$user = 'admin'){
    yts_notice($message,$type,'Large',$title, $helpurl, 'echo');    
}
...
* @param mixed $user
*/
function yts_n_postresult($type,$title,$message,$helpurl = false,$user = 'admin'){
    yts_notice($message,$type,'Large',$title, $helpurl, 'echo');    
}
...
*/
function yts_n_screeninfo($title,$message,$helpurl = false,$user = 'admin'){
    echo yts_notice($message,'info','Tiny',$title, $helpurl, 'return');    
}

/**
* Standard notice format to be displayed inside a panel as information
*/
function yts_n_panelinfo($title,$message,$helpurl = false,$user = 'admin'){
    echo yts_notice($message,'info','Small',$title, $helpurl, 'return');    
}

/**
* Standard notice format for displaying notice on public side
*/
function yts_n_public($title,$message,$helpurl = false){
    echo yts_n($title,$message,'info','Small',array('output' => 'public','url' => $helpurl));    
}

/**
* Notice html is printed where this function is used 
*/
function yts_n_incontent($message,$type = 'info',$size = 'Small',$title = '',$helpurl = ''){
    echo yts_notice($message,$type,$size,$title, $helpurl, 'return');    
}
...
*/
function yts_notice_display_step($helpurl,$title,$message){
    $output = '<div class="stepLargeTest"><a href="'.$helpurl.'">    
...
* 104857600 = 100 MB 
*/
function yts_notice_filesizetotal($mb){
    if($mb > 104857600){
        echo yts_notice('Your CSV files combined size is larger than 100MB, it is recommended that you delete any files
...
*/
function yts_var_dump($v,$h = false){
...
/**
* YouTube Sidebar function for logging project changes.
...
*/
function yts_log_posts($post_id,$post_title,$action,$message,$project_name){
    $atts = array();   
    $atts['logged'] = yts_date();
    $atts['postid'] = $post_id;
...
*/
function yts_log_schedule($action,$outcome,$trigger = 'schedule',$line = 'NA',$file = 'NA',$function = 'NA'){
    $atts = array();   
    $atts['logged'] = yts_date();
    $atts['action'] = $action;
...
/**
* Make log entries into the data log file, a log file specific to YouTube Sidebar. 
* This function is not to be moved to the wtgcore. 
*/
function yts_log_data($comment,$line,$file,$function,$dump = 'NA',$sqlresult = 'NA',$sqlquery = 'NA',$lastrecordedid = 'NA',$jobid = 'NA'){
    $atts = array();   
...
*/
function yts_get_option_persistentnotifications_array(){
    $a = get_option('yts_notifications');
...
*/
function yts_log($atts){     
    global $yts_adm_set,$wpdb,$yts_currentversion;

    // if ALL logging is off - if ['uselog'] not set then logging for all files is on by default
    if(isset($yts_adm_set['reporting']['uselog']) && $yts_adm_set['reporting']['uselog'] == 0){return false;}
    
    // if log table does not exist return false
    if(!yts_WP_SQL_does_table_exist('yts_log')){return false;}
...
*/
function yts_log_adminform($function,$outcome,$action,$comment = 'unknown',$type = 'general',$priority = 'normal',$dump = 'unknown',$wordpresserror = 'none'){    
    ...
    $atts['page'] = $_POST['yts_hidden_pageid'];
    $atts['panelname'] = $_POST['yts_hidden_panel_name'];
    $atts['paneltitle'] = $_POST['yts_hidden_panel_title'];
    $atts['tabscreenid'] = $_POST['yts_hidden_tabnumber'];                              
            
    yts_log($atts);     
}

function yts_log_urlaction($function,$outcome,$action,$comment = 'unknown',$type = 'general',$priority = 'normal',$dump = 'unknown',$wordpresserror = 'none'){    
...
*/
function yts_log_sql($comment,$function,$file,$line,$recordid = 'NA',$dump = 'NA',$sqlresult = 'NA',$sqlquery = 'NA',$outcome = 'success'){
    $atts = array();             
...
* Place the yts_log_user() function in form processing.
*/
function yts_log_users($message,$ipaddress = 'NA',$userid = 'NA',$panelname = 'NA',$paneltitle = 'NA',$page = 'NA'){
    $atts = array();              
    $atts['userid'] = $userid;
    $atts['message'] = $message;
...
*/
function yts_log_error($message,$line,$function,$file,$dump = 'NA'){
    $atts = array();             
    $atts['line'] = $line;
    $atts['function'] = $function;
    $atts['file'] = $file;
...
*/
function yts_log_install($action,$message,$userid = 'NA'){
    $atts = array();             
    $atts['action'] = $action;
    $atts['userid'] = $userid;
    $atts['message'] = $message;
