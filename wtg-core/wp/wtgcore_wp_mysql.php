...
*/
function yts_option($option,$action,$value = 'No Value',$autoload = 'yes'){
    if($action == 'add'){  
...
*/
function yts_WP_SQL_select($table_name,$limit = 10,$columns = '*',$where = ''){
    global $wpdb;    
...
function yts_WP_SQL_log_queryactions(){
    global $wpdb;    
    return $wpdb->get_results( 'SELECT DISTINCT action FROM yts_log',ARRAY_A );    
}

/**
* Gets posts with the giving meta value
*/
function yts_WP_SQL_get_posts_join_meta($meta_key,$meta_value,$limit = 1,$select = '*',$where = '',$query = 'get_results'){
    global $wpdb;
...
* @return string|null
*/
function yts_WP_SQL_get_post_ID_by_postname($name){
    global $wpdb;
...
*/
function yts_WP_SQL_return_cols_with_data_type($tableName,$dataType = false){
    global $wpdb;
...
function yts_WP_SQL_querylog_bytype($type = 'all',$limit = 100){
    global $wpdb;
...
*/
function yts_WP_SQL_do_tables_exist($tables_array){
    if($tables_array && is_array($tables_array)){         
        ...
        foreach($tables_array as $key => $table_name){
            $table_exists = yts_WP_SQL_does_table_exist($table_name);  
            if(!$table_exists){          
...
*/
function yts_WP_SQL_column_distinctvalues($table_name,$column_name){
    global $wpdb;
...
*/
function yts_WP_SQL_get_customfield_keys_distinct(){
    global $wpdb;
...
* Uses get_results and finds all DISTINCT meta_keys, returns the result  
*/
function yts_WP_SQL_metakeys_distinct(){
    global $wpdb;
    return $wpdb->get_results("SELECT DISTINCT meta_key FROM $wpdb->postmeta 
                                  WHERE meta_key != '_encloseme' 
                                  AND meta_key != '_wp_page_template'
...
*/
function yts_WP_SQL_count_records( $table_name ){
    return yts_WP_SQL_counttablerecords( $table_name );
}
 ...
 */
function yts_WP_SQL_counttablerecords( $table_name,$where = '' ){
    global $wpdb;
...
*/
function yts_WP_SQL_options_beginning_with($prependvalue){
    ...
    // loop through each option record and check their name value for yts_ at the beginning
 ...
 */
function yts_WP_SQL_does_table_exist( $table_name ){
    global $wpdb;                                      
...
*/
function yts_WP_SQL_get_tables(){
    global $wpdb;
...
*/
function yts_WP_SQL_get_tablecolumns($t,$return_array = false,$columns_only = false){
    global $wpdb;
...
* @return mixed
*/
function yts_WP_SQL_is_categorywithparent( $category_term,$parent_id ){
    global $wpdb;
...
* @todo LOWPRIORITY
*/
function yts_WP_SQL_formatter($sql_query){
    return $sql_query;    
}
 ...
 */
function yts_database_table_exist( $table_name ){
    global $wpdb;
...
* @param mixed $table_name
*/
function yts_WP_SQL_drop_table($table_name){
    global $wpdb;
    $r = $wpdb->query("DROP TABLE IF EXISTS ".$table_name."");
    if( $r ){
        yts_notice('Database table named '.$table_name.' has been deleted.','success','Tiny','','','echo');
    }else{
