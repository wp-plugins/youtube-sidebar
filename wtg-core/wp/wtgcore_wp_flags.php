...
*/
function yts_flag($atts){
    if(!yts_WP_SQL_does_table_exist('yts_flags')){
        return false;    
    }   
...
/**
* Flags the giving post by adding _yts_flagged meta value which is used in the flagging system.
*     
...
*/
function yts_flag_post($post_ID,$priority,$type,$reason){
    $a = array();
    $a['priority'] = $priority;
    $a['time'] = time();
    $a['reason'] = $reason;
    ...
    add_post_meta($post_ID,'_yts_flagged',$a,false);   
}  
?>
