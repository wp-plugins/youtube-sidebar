...
*/
function yts_WP_SQL_build_columnstring($table_name){
    $table_columns = yts_WP_SQL_get_tablecolumns($table_name);
    $column_string = '';
    $first_column = true;
...
*/
function yts_clean_string($string){ 
...
*/
function yts_create_code($length = 10,$specialchars = false) { 
...
* @param string $text
*/
function yts_pluralize( $count, $text ) { 
...
*/
function yts_commas($originalstring){
...
* @todo                           
*/
function yts_get_files_age($time){
               
    //echo date("F d Y H:i:s.", filemtime(WTG_YTS_CONTENTFOLDER_DIR .'/'. $filename));
...
*/
function yts_ago( $datetime,$use_year = true,$use_month = true,$use_day = true,$use_hour = true,$use_minute = true,$use_second = false ){
    ...
    if($use_year){
        if ( $interval->y >= 1 ){
            $ago_string .= yts_pluralize( $interval->y, 'year' );        
        } 
    }
    ...
    if($use_month){
        if ( $interval->m >= 1 ){
            $ago_string_with_comma_month = yts_commas($ago_string); 
            $ago_string = $ago_string_with_comma_month . yts_pluralize( $interval->m, 'month' );        
        } 
    }  
    ...
    if($use_day){
        if ( $interval->d >= 1 ){
            $ago_string_with_comma_day = yts_commas($ago_string);            
            $ago_string = $ago_string_with_comma_day . yts_pluralize( $interval->d, 'day' );        
        } 
    }
    ...
    if($use_hour){
        if ( $interval->h >= 1 ){
            $ago_string_with_comma_hour = yts_commas($ago_string);            
            $ago_string = $ago_string_with_comma_hour . yts_pluralize( $interval->h, 'hour' );        
        } 
    }       
    ...
    if($use_hour){
        if ( $interval->m >= 1 ){
            $ago_string_with_comma_minute = yts_commas($ago_string);            
            $ago_string = $ago_string_with_comma_minute . yts_pluralize( $interval->m, 'minute' );        
        } 
    }
    ...
    if($use_second){
        if ( $interval->s >= 1 ){
            $ago_string_with_comma_second = yts_commas($ago_string);            
            $ago_string = $ago_string_with_comma_second . yts_pluralize( $interval->s, 'second' );        
        } 
    }    
...
*/
function yts_truncatestring( $string, $max_length ){
 ...
 */
function yts_create_username($username_base){
    $attempt = 0;
...
/**
* Wrapper, uses yts_link_toadmin to create local admin url
* 
* @param mixed $page
* @param mixed $values 
*/
function yts_create_adminurl($page,$values = ''){
    return yts_link_toadmin($page,$values);    
}
...
*/
function yts_date($timeaddition = 0,$time = false){
    $thetime = time();
...
* 
*/
function yts_contactscreen_url(){
    //return get_admin_url(null,'admin.php?page=yts_more&yts_tab=tab10_more');   OLD TAB METHOD     
    return get_admin_url(null,'admin.php?page=yts_more#tabs-9');// new jquery tabs method
}  
?>