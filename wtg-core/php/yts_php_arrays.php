...
* @param mixed $version
*/
function yts_ARRAY_arrayinfo_set($line,$function,$file,$reason = false,$version = false){
    $a = array();
...
*/
function yts_ARRAYS_random_value($array,$number = 1){
...
*/
function yts_ARRAYS_get_next_value($array,$current_value){
    ...
    if(!$current_value_key || !is_numeric($current_value_key)){
        return yts_ARRAYS_random_value($array);
    }
    
    $next_key = $current_value_key + 1;

    if(!isset($array[$next_key])){
        return yts_ARRAYS_random_value($array);// key is missing so return a random value instead    
    }
 ...
 */
function yts_option_array_update( $option_key, $option_value, $line = __LINE__, $file = __FILE__, $function = __FUNCTION__ ){
 ...
 */
function yts_oddeven_arrayelements($array){
