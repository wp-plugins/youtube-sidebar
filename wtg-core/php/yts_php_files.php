 ...
 */
function yts_createfolder($path,$chmod = 0700){
...
* 2. Faster than yts_count_extension_in_directory() because it returns sooner, important for directory with a lot of files
* 
* @returns boolean
*/
function yts_does_folder_contain_file_type($path,$extension){
...
* 1. Use yts_does_folder_contain_file_type() for checking if any files with specific extension exist especially if directory contains many files
* 
*/
function yts_count_extension_in_directory(){
    $all_files  = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path));
    $html_files = new RegexIterator($all_files, '/\.'.$extension.'/');
    $count = 0;                        
    foreach($html_files as $file) {
