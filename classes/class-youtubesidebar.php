...
class YOUTUBESIDEBAR {
    ...
    /**
     * Page hooks (i.e. names) WordPress uses for the YOUTUBESIDEBAR admin screens,
    ...
    protected
        ...
        $plugin_actions = array( 
            ...
            array( 'plugins_loaded',                 'widget_register_basic',                                  'all' ),            
            array( 'plugins_loaded',                 'widget_register_advanced',                               'all' ),            
        ),        
    ...
    */
    public function __construct() {
        ...
        $this->DB = self::load_class( 'YOUTUBESIDEBAR_DB', 'class-wpdb.php', 'classes' );
        $this->PHP = self::load_class( 'YOUTUBESIDEBAR_PHP', 'class-phplibrary.php', 'classes' );
        $this->Install = self::load_class( 'YOUTUBESIDEBAR_Install', 'class-install.php', 'classes' );
        $this->Files = self::load_class( 'YOUTUBESIDEBAR_Files', 'class-files.php', 'classes' );
  
        $youtubesidebar_settings = self::adminsettings();
        ...
        if( is_admin() ){
            ...
            // admin globals 
            global $youtubesidebar_notice_array;
            
            $youtubesidebar_notice_array = array();// set notice array for storing new notices in (not persistent notices)
            
            // load class used from admin only                   
            $this->UI = self::load_class( 'YOUTUBESIDEBAR_UI', 'class-ui.php', 'classes' );
            $this->Helparray = self::load_class( 'YOUTUBESIDEBAR_Help', 'class-help.php', 'classes' );        
    ...
    */
    public function set_admin_globals() {
        global $youtubesidebar_menu_array;
        
        // set menu array
        $YOUTUBESIDEBAR_TabMenu = self::load_class( 'YOUTUBESIDEBAR_TabMenu', 'class-pluginmenu.php', 'classes' );
        $youtubesidebar_menu_array = $YOUTUBESIDEBAR_TabMenu->menu_array();   
        
        // set page name (it's my own approach, each tab/view has a name which is shorter than the WP view ID)
        $youtubesidebar_page_name = self::get_admin_page_name();             
    }
    ...
    */
    public function plugin_admin_register_styles() {
        wp_register_style( 'youtubesidebar_css_notification',plugins_url( 'youtubesidebar/css/notifications.css' ), array(), '1.0.0', 'screen' );
        wp_register_style( 'youtubesidebar_css_admin',plugins_url( 'youtubesidebar/css/admin.css' ), __FILE__);          
    }
    ...
    */
    public function plugin_admin_print_styles() {
        wp_enqueue_style( 'youtubesidebar_css_notification' );  
        wp_enqueue_style( 'youtubesidebar_css_admin' );               
    }    
    ...
    */
    ...
    public function enqueue_style( $name, array $dependencies = array() ) {
        ...
        $css_url = plugins_url( $css_file, YOUTUBESIDEBAR__FILE__ );
        wp_enqueue_style( "youtubesidebar-{$name}", $css_url, $dependencies, YOUTUBESIDEBAR::version );
    }
    ...
    public function enqueue_script( $name, array $dependencies = array(), $localize_script = false, $force_minified = false ) {
        ...
        $js_url = plugins_url( $js_file, YOUTUBESIDEBAR__FILE__ );
        wp_enqueue_script( "youtubesidebar-{$name}", $js_url, $dependencies, YOUTUBESIDEBAR::version, true );
    }  
    ...
    */
    public static function load_class( $class, $file, $folder, $params = null ) {
         ...
         */        
        $class = apply_filters( 'youtubesidebar_load_class_name', $class );
    ...
    /**
    * returns the YOUTUBESIDEBAR_WPMain class object already created in this YOUTUBESIDEBAR class
    * 
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    * @since 3.0.0
    ...
    */
    ...
    public static function load_file( $file, $folder ) {      
        $full_path = YOUTUBESIDEBAR_ABSPATH . $folder . '/' . $file;
         ...
         */
        $full_path = apply_filters( 'youtubesidebar_load_file_full_path', $full_path, $file, $folder );
        if ( $full_path ) {   
    ...
    public static function load_draggableboxes_view( $page_slug, array $data = array() ) {
        ...
        require_once( YOUTUBESIDEBAR_ABSPATH . 'classes/class-view.php' );
        ...
        $the_view = self::load_class( "YOUTUBESIDEBAR_{$ucview}_View", "{$page_slug}.php", $dir );
     ...
     */
     public function load_admin_page() {
        // remove "youtubesidebar_" from page value in URL which leaves the page name as used in the menu array
        $page = 'main';
        if( isset( $_GET['page'] ) && $_GET['page'] !== 'youtubesidebar' ){    
            $page = substr( $_GET['page'], strlen( 'youtubesidebar_' ) );
        }
    ...
    */
    private function filteraction_should_beloaded( $whenToLoad) {
        $youtubesidebar_settings = $this->adminsettings();
        ...
        switch( $whenToLoad) {
            ...
            case 'pluginscreens':
                ...
                // load when on a YouTube Sidebar admin screen
                if( isset( $_GET['page'] ) && strstr( $_GET['page'], 'youtubesidebar' ) ){return true;}
            ...
            case 'pluginanddashboard':
                ...
                if( isset( $_GET['page'] ) && strstr( $_GET['page'], 'youtubesidebar' ) ){
                    return true;
                }
            ...
            case 'systematicpostupdating':  
                if(!isset( $youtubesidebar_settings['standardsettings']['systematicpostupdating'] ) || $youtubesidebar_settings['standardsettings']['systematicpostupdating'] != 'enabled' ){
                    return false;    
                }      
                return true;
            break;
    ...
    */
    public function debugmode() {
        global $youtubesidebar_debug_mode;
        if( $youtubesidebar_debug_mode){
            global $wpdb;
    ...
    */
    public function adminsettings() {
        $result = $this->option( 'youtubesidebar_settings', 'get' );
    ...
    * 
    * @uses YOUTUBESIDEBAR_UI::add_dashboard_widgets() which has the widgets
    * 
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    * @since 3.0.0
    ...
    */       
    ...
    public function screen_options() {
        ...
        // toplevel_page_youtubesidebar (main page)
        if( $screen->id == 'toplevel_page_youtubesidebar' ){
            $args = array(
                'label' => __( 'Members per page' ),
                'default' => 1,
                'option' => 'youtubesidebar_testoption'
            );
    ...
    public function save_screen_option( $status, $option, $value ) {
        if ( 'youtubesidebar_testoption' == $option ) return $value;
    }
    ...
    */
    public function help_tab () {
        ...
        $YOUTUBESIDEBAR_Help = self::load_class( 'YOUTUBESIDEBAR_Help', 'class-help.php', 'classes' );

        // call the array
        $help_array = $YOUTUBESIDEBAR_Help->get_help_array();
        ...
        // get page name i.e. youtubesidebar_page_youtubesidebar_affiliates would return affiliates
        $page_name = $this->PHP->get_string_after_last_character( $screen->id, '_' );
        
        // if on main page "youtubesidebar" then set tab name as main
        if( $page_name == 'youtubesidebar' ){$page_name = 'main';}
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'You are welcome to visit the', 'youtubesidebar' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] . '"';
            $help_content .= 'title="' . __( 'Visit the YouTube Sidebar website and read more about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';
            $help_content .= __( 'YouTube Sidebar Website', 'youtubesidebar' ) . '</a> ' . __( 'to read more about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'There is a', 'youtubesidebar' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] . '"';
            $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= __( 'YouTube Video', 'youtubesidebar' ) . '</a> ' . __( 'about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewdiscussurl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'We invite you to take discuss', 'youtubesidebar' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewdiscussurl' ] . '"';
            $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'youtubesidebar' );           
            $help_content .= '.</p>';
        }         
        ...
        $screen->add_help_tab( array(
            'id'    => $page_name,
            'title'    => __( 'About', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] ,
        ...
        foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ] as $form_id => $value ){
            ...
            if( isset( $value[ 'formreadmoreurl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Visit the YouTube Sidebar website and read more about', 'youtubesidebar' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';
                $help_content .= __( 'YouTube Sidebar Website', 'youtubesidebar' ) . '</a> ' . __( 'to read more about', 'youtubesidebar' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }  
            ...
            if( isset( $value[ 'formvideourl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'youtubesidebar' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= __( 'YouTube Video', 'youtubesidebar' ) . '</a> ' . __( 'about', 'youtubesidebar' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }
            ...
            if( isset( $value[ 'formdiscussurl' ] ) ){
                $help_content .= '<p>';
                $help_content .= __( 'We invite you to discuss', 'youtubesidebar' ) . ' ';
                $help_content .= '<a href="' . $value[ 'formdiscussurl' ] . '"';
                $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'youtubesidebar' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= $value[ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'youtubesidebar' );           
                $help_content .= '.</p>';
            } 
            ...
            foreach( $value[ 'options' ] as $key_two => $option_array ){  
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $help_content .= ' title="' . __( 'Read More about', 'youtubesidebar' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Read More', 'youtubesidebar' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $help_content .= ' title="' . __( 'Watch a video about', 'youtubesidebar' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Video', 'youtubesidebar' ) . '</a>';      
                }
            }
    ...
    * @version 1.0
    *  
    * @param mixed $youtubesidebar_page_name
    * @param mixed $default
    */
    ...
    */
    public function admin_menu() {    
        ...
        $YOUTUBESIDEBAR_Menu = $youtubesidebar_menu_array;
        ...
        // add menu
        $this->page_hooks[] = add_media_page( $YOUTUBESIDEBAR_Menu['main']['title'], 
        __( 'YouTube Sidebar', 'youtubesidebar' ), 
        'administrator', 
        'youtubesidebar',  
        $subpage_callback ); 
        
        // help tab                                                 
        add_action( 'load-toplevel_page_youtubesidebar', array( $this, 'help_tab' ) );
        ...
        foreach( $YOUTUBESIDEBAR_Menu as $key_pagename => $page_array ){ 
        ...
        foreach( $YOUTUBESIDEBAR_Menu as $key_pagename => $page_array ){                 
            ...
            if( !isset( $_GET['page'] ) || !strstr( $_GET['page'], 'youtubesidebar' ) ){
                ...
                // remove none parents
                if( $page_array['parent'] !== 'parent' ){    
                    unset( $YOUTUBESIDEBAR_Menu[ $key_pagename ] ); 
                }        
            
            }elseif( isset( $_GET['page'] ) && strstr( $_GET['page'], 'youtubesidebar' ) ){
            ...
            if( isset( $_GET['page'] ) && $page_array['slug'] === $_GET['page'] ){
                unset( $YOUTUBESIDEBAR_Menu[ $YOUTUBESIDEBAR_Menu[ $key_pagename ]['parent'] ] );
            }
            
            // remove update page as it is only meant to show when new version of files applied
            if( $page_array['slug'] == 'youtubesidebar_pluginupdate' ) {
                unset( $YOUTUBESIDEBAR_Menu[ $key_pagename ] );
            }
        }
        ...
        foreach( $YOUTUBESIDEBAR_Menu as $key_pagename => $page_array ){ 
            ...
            $new_hook = add_submenu_page( 'youtubesidebar', 
                   $group_titles_array[ $page_array['groupname'] ]['grouptitle'], 
                   $group_titles_array[ $page_array['groupname'] ]['grouptitle'], 
                   self::get_page_capability( $key_pagename ), 
                   $YOUTUBESIDEBAR_Menu[ $key_pagename ]['slug'], 
                   $subpage_callback );     
         
            $this->page_hooks[] = $new_hook;
                   
            // help tab                                                 
            add_action( 'load-youtubesidebar_page_youtubesidebar_' . $key_pagename, array( $this, 'help_tab' ) );              
        }
    }
    ...
    * 
    * function was reduced to two lines, the contents mode to YOUTUBESIDEBAR_Requests itself.
    *
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    * @since 3.0.0
    ...
    */
    public function process_admin_POST_GET() {
        ...
        if( isset( $_POST['youtubesidebar_admin_action'] ) ) {
            $early_deny_request = false;
                
            // run nonce security for a form submission
            if( !isset( $_POST['youtubesidebar_form_name'] ) ) {    
                return false;
            }

            check_admin_referer( $_POST['youtubesidebar_form_name'] );# exists here if failed
            $function = $_POST['youtubesidebar_form_name'];
        ...
        if( isset( $_GET['youtubesidebaraction'] )  ) {
               
            check_admin_referer( $_GET['youtubesidebaraction'] );# exists here if failed
            $function = $_GET['youtubesidebaraction'];  
        ...
        if( $method !== 'unknown' ) {   
            $YOUTUBESIDEBAR_REQ = self::load_class( 'YOUTUBESIDEBAR_Requests', 'class-requests.php', 'classes' );
            $YOUTUBESIDEBAR_REQ->process_admin_request( $method, $function );
        }
    } 
    ...
    */
    public function pluginmediabutton_popup() {
        ...
        <div id="youtubesidebar_popup_container" style="display:none;">
    ...
    /**
    * Basic sidebar widget registration
    *
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    * @since 3.0.0
    * @version 1.0
    */
    public function widget_register_basic() {
        $widget_ops = array('classname' => 'youtubesidebarwidgetbasic', 'description' => "Display one video or one ad with default options from video or ad sources" );
        wp_register_sidebar_widget('youtubesidebarwidgetbasic', 'YouTube Sidebar: Basic', array( $this, 'widget_basic' ), $widget_ops);    
    }
    
    /**
    * Advanced sidebar widget registration
    *
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    * @since 3.0.0
    * @version 1.0
    */
    public function widget_register_advanced() {
        $widget_ops = array('classname' => 'youtubesidebarwidgetadvanced', 'description' => "Display multiple videos and/or ads with all options applied" );
        wp_register_sidebar_widget('youtubesidebarwidgetadvanced', 'YouTube Sidebar: Advanced', array( $this, 'widget_advanced' ), $widget_ops);    
    }
    ...
    * @version 1.0
    * 
    * @todo validate the YouTube ID, flag the entry if invalid 
    */
    public function widget_basic( $args ) {
            ...
            if ( have_posts() ){   
                ...
                while ( have_posts() ){
                    ...
                    if( $video_count == 0  ){
                        ...
                        foreach( $youtubesidebar_settings['adsnippets'] as $key => $ad ){ 
    ...
    * @todo validate the YouTube ID, flag the entry if invalid 
    */
    public function widget_advanced( $args ) {
                ...
                while ( have_posts() ){
                    ...
                    if( $video_codes ){
                        foreach( $video_codes as $youtubeid ){
                            if( is_string( $youtubeid ) && $youtubeid != '' ){
                                $args = array();
                                $args['color'] = $youtubesidebar_settings['videooptions']['color'];
                                $args['border'] = $youtubesidebar_settings['videooptions']['border'];
                                $args['autoplay'] = $youtubesidebar_settings['videooptions']['autoplay'];
                                $args['fullscreen'] = $youtubesidebar_settings['videooptions']['fullscreen'];
                                self::video_object( $youtubeid );                        
    ...
    * @version 1.0
    */
    public function video_object( $videoid, $args = array() ) {
        $args = array (
             'width' => '',
             'height' => '',
             'color' => '',
    ...
    * 
    * Debugging Trace
    * $youtubesidebar_schedule_array['history']['trace'] is used to indicate how far the this script went before a return.
    ...
    */
    public function event_check() {
        $youtubesidebar_schedule_array = self::get_option_schedule_array();
        
        // do not continue if WordPress is DOING_AJAX
        if( self::request_made() ){return;}
                      
        self::log_schedule( __( 'The schedule is being checked. There should be further log entries explaining the outcome.', 'youtubesidebar' ), __( 'schedule being checked', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
        ...
        //  do not use youtubesidebar_event_refused as we do not want to set the array
        if(!isset( $youtubesidebar_schedule_array ) || !is_array( $youtubesidebar_schedule_array ) ){       
            self::log_schedule( __( 'Scheduled events cannot be peformed due to the schedule array of stored settings not existing.', 'youtubesidebar' ), __( 'schedule settings missing', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            return false;
        }
        ...
        if( isset( $youtubesidebar_schedule_array['history']['lasteventtime'] ) )
        {    
            // increase lasteventtime by 60 seconds
            $soonest = $youtubesidebar_schedule_array['history']['lasteventtime'] + 60;//hack info page http://www.webtechglobal.co.uk/hacking/increase-automatic-events-delay-time
            
            if( $soonest > time() ){
                self::log_schedule( __( 'No changed made as it has not been 60 seconds since the last event.', 'youtubesidebar' ), __( 'enforcing schedule event delay', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
                self::event_return( __( 'has not been 60 seconds since list event', 'youtubesidebar' ) ); 
                return;
            }             
        }
        else
        {               
            ...
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The last even time event was set for the first time, no further processing was done.', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            ...
            changes are made during this schedule check.', 'youtubesidebar' ), __( 'initialized schedule delay timer', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);

            self::event_return( __( 'initialised the last event time value', 'youtubesidebar' ) );
            return;        
        }                             
                                           
        // is last event type value set? if not set default as dataupdate, this means postcreation is the next event
        if(!isset( $youtubesidebar_schedule_array['history']['lasteventtype'] ) )
        {    
            $youtubesidebar_schedule_array['history']['lasteventtype'] = 'dataupdate';
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The last event type value was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            ...
            determine what event comes next.', 'youtubesidebar' ), __( 'initialized schedule last event value', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
            self::event_return( __( 'initialised last event type value', 'youtubesidebar' ) );
            return;
        }
                 
        // does the "day_lastreset"" time value exist, if not we set it now then return
        if(!isset( $youtubesidebar_schedule_array['history']['day_lastreset'] ) )
        {    
            $youtubesidebar_schedule_array['history']['day_lastreset'] = time();
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The last daily reset time was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            ...
            and should only happen once.', 'youtubesidebar' ), __( '24 hour timer set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
            self::event_return( __( 'initialised last daily reset time', 'youtubesidebar' ) );        
            return;
        } 
                                                         
        // does the "hour_lastreset"" time value exist, if not we set it now then return
        if(!isset( $youtubesidebar_schedule_array['history']['hour_lastreset'] ) )
        { 
            $youtubesidebar_schedule_array['history']['hour_lastreset'] = time();
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The hourly reset time was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            ...
            taking. This should only happen once.', 'youtubesidebar' ), __( 'one hour timer set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);        
            
            self::event_return( __( 'initialised hourly reset time', 'youtubesidebar' ) );
            return;
        }    
               
        // does the hourcounter value exist, if not we set it now then return (this is to initialize the variable)
        if(!isset( $youtubesidebar_schedule_array['history']['hourcounter'] ) )
        {     
            $youtubesidebar_schedule_array['history']['hourcounter'] = 0;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The hourly events counter was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            self::log_schedule( __( 'Number of events per hour has been set for the first time, this change is treated as an event.', 'youtubesidebar' ), __( 'hourly events counter set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);     
            self::event_return( __( 'initialised hourly events counter', 'youtubesidebar' ) );   
            return;
        }     
                                     
        // does the daycounter value exist, if not we set it now then return (this is to initialize the variable)
        if(!isset( $youtubesidebar_schedule_array['history']['daycounter'] ) )
        {
            $youtubesidebar_schedule_array['history']['daycounter'] = 0;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'The daily events counter was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            self::log_schedule( __( 'The daily events counter was not set. No further action was taking. This measure should only happen once.', 'youtubesidebar' ), __( 'daily events counter set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);     
            self::event_return( __( 'initialised daily events counter', 'youtubesidebar' ) );           
            return;
        } 
        ...
        $hour_reset_time = $youtubesidebar_schedule_array['history']['hour_lastreset'] + 3600;
        if(time() > $hour_reset_time )
        {     
            // reset hour_lastreset value and the hourlycounter
            $youtubesidebar_schedule_array['history']['hour_lastreset'] = time();
            $youtubesidebar_schedule_array['history']['hourcounter'] = 0;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'Hourly counter was reset for another 60 minute period', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            self::log_schedule( __( 'Hourly counter has been reset, no further action is taking during this event. This should only happen once every hour.', 'youtubesidebar' ), __( 'hourly counter reset', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'hourly counter was reset', 'youtubesidebar' ) );        
            return;
        }  
        ...
        $day_reset_time = $youtubesidebar_schedule_array['history']['day_lastreset'] + 86400;
        if(time() > $day_reset_time )
        {
            ...
            $youtubesidebar_schedule_array['history']['hourcounter'] = 0;
            $youtubesidebar_schedule_array['history']['daycounter'] = 0;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'Daily and hourly events counter reset for a new 24 hours period', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array ); 
            self::log_schedule( __( '24 hours had passed and the daily counter had to be reset. No further action is taking during these events and this should only happen once a day.', 'youtubesidebar' ), __( 'daily counter reset', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);   
        ...
        if(!isset( $youtubesidebar_schedule_array['days'][$day] ) )
        {
            self::event_return( __( 'Event processing is has not been permitted for today', 'youtubesidebar' ) );
            self::log_schedule( __( 'Event processing is not permitted for today. Please check schedule settings to change this.', 'youtubesidebar' ), __( 'schedule not permitted today', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
        ...
        if(!isset( $youtubesidebar_schedule_array['hours'][$hour] ) )
        {
            self::event_return( __( 'Event processing is has not been permitted for the hour', 'youtubesidebar' ) );
            self::log_schedule( __( 'Processsing is not permitted for the current hour. Please check schedule settings to change this.', 'youtubesidebar' ), __( 'hour not permitted', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'schedule not permitting hour', 'youtubesidebar' ) );        
            return;    
        }

        // ensure hourly limit value has been set
        if(!isset( $youtubesidebar_schedule_array['limits']['hour'] ) )
        {  
            $youtubesidebar_schedule_array['limits']['hour'] = 1;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'Hourly limit was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            self::log_schedule( __( 'The hourly limit value had not been set yet. You can change the limit but the default has been set to one. No further action is taking during this event and this should only happen once.', 'youtubesidebar' ), __( 'no hourly limit set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'initialised hourly limit', 'youtubesidebar' ) );        
            return;
        }     
                    
        // ensure daily limit value has been set
        if(!isset( $youtubesidebar_schedule_array['limits']['day'] ) )
        {
            $youtubesidebar_schedule_array['limits']['day'] = 1;
            $youtubesidebar_schedule_array['history']['lastreturnreason'] = __( 'Daily limit was set for the first time', 'youtubesidebar' );
            self::update_option_schedule_array( $youtubesidebar_schedule_array );
            self::log_schedule( __( 'The daily limit value had not been set yet. It has now been set as one which allows only one post to be created or updated etc. This action should only happen once.', 'youtubesidebar' ), __( 'no daily limit set', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__); 
            self::event_return( __( 'initialised daily limit', 'youtubesidebar' ) );           
            return;
        }

        // if this hours target has been met return
        if( $youtubesidebar_schedule_array['history']['hourcounter'] >= $youtubesidebar_schedule_array['limits']['hour'] )
        {
            self::event_return( 'The hours event limit/target has been met' );
            self::log_schedule( __( 'The events target for the current hour has been met so no further processing is permitted.', 'youtubesidebar' ), __( 'hourly target met', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'hours limit reached', 'youtubesidebar' ) );            
            return;        
        }
         
        // if this days target has been met return
        if( $youtubesidebar_schedule_array['history']['daycounter'] >= $youtubesidebar_schedule_array['limits']['day'] )
        {
            self::event_return( __( 'The days event limit/target has been met', 'youtubesidebar' ) );
            self::log_schedule( __( 'The daily events target has been met for the current 24 hour period (see daily timer counter). No events will be processed until the daily timer reaches 24 hours and is reset.', 'youtubesidebar' ), __( 'daily target met', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'days limit reached', 'youtubesidebar' ) );        
            return;       
        }
        ...
        self::log_schedule(sprintf( __( 'The schedule system decided that the next event type is %s.', 'youtubesidebar' ), $run_event_type), __( 'next event type determined', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
        // update $youtubesidebar_schedule_array with decided event type to advance the cycle and increase hourly plus daily counter
        $youtubesidebar_schedule_array['history']['lasteventtype'] = $run_event_type;
        $youtubesidebar_schedule_array['history']['lasteventtime'] = time(); 
        $youtubesidebar_schedule_array['history']['hourcounter'] = $youtubesidebar_schedule_array['history']['hourcounter'] + 1; 
        $youtubesidebar_schedule_array['history']['daycounter'] = $youtubesidebar_schedule_array['history']['daycounter'] + 1;
        self::update_option_schedule_array( $youtubesidebar_schedule_array );
    ...
    * youtubesidebar solution. Passing value to this method, can be used to call more specific method
    * for an event.
    * 
    * @author Ryan R. Bayne
    * @package YouTube Sidebar
    ...
    * Must call ths function within a function that checks that uses youtubesidebar_DOING_AJAX() first 
    * 
    * 1. If you add a new event type, you must also update youtubesidebar_tab1_pagecreation.php (Schedule), specifically Events Status panel
    * 2. Update event_action when adding a new event type
    * 3. Update the Event Types panel and add option for new event types
    * 4. Update youtubesidebar_form_save_eventtypes
    * 
    * @link http://www.webtechglobal.co.uk/hacking/event-types
    */
    public function event_decide() {
        global $youtubesidebar_schedule_array, $youtubesidebar_settings;
        ...
        {
            self::log_schedule(sprintf( __( 'The plugins ability to override the next due event type has been applied and then next event forced is %s.', 'youtubesidebar' ), $override_event), __( 'next event type override', 'youtubesidebar' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);         
        ...
        if( $youtubesidebar_schedule_array['history']['lasteventtype'] == 'dataimport' ){
            if( isset( $youtubesidebar_schedule_array['eventtypes']['dataupdate']['switch'] ) && $youtubesidebar_schedule_array['eventtypes']['dataupdate']['switch'] == true){
                 return 'dataupdate';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $youtubesidebar_schedule_array['history']['lasteventtype'] == 'dataupdate' || $bypass == true){
            if( isset( $youtubesidebar_schedule_array['eventtypes']['postcreation']['switch'] ) && $youtubesidebar_schedule_array['eventtypes']['postcreation']['switch'] == true){
                 return 'postcreation';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $youtubesidebar_schedule_array['history']['lasteventtype'] == 'postcreation' || $bypass == true){
            if( isset( $youtubesidebar_schedule_array['eventtypes']['postupdate']['switch'] ) && $youtubesidebar_schedule_array['eventtypes']['postupdate']['switch'] == true){
                return 'postupdate';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $youtubesidebar_schedule_array['history']['lasteventtype'] == 'postupdate' || $bypass == true){
            if( isset( $youtubesidebar_schedule_array['eventtypes']['dataimport']['switch'] ) && $youtubesidebar_schedule_array['eventtypes']['dataimport']['switch'] == true){
                 return 'dataimport';    
            }else{
                $bypass = true; 
            }            
    ...
    */
    public function event_decide_focus() {
        $youtubesidebar_schedule_array = self::get_option_schedule_array();
        if( isset( $youtubesidebar_schedule_array['focus'] ) && $youtubesidebar_schedule_array['focus'] != false ){
            return $youtubesidebar_schedule_array['focus'];    
        }
    }
    ...
    */
    public function event_action( $run_event_type){    
        global $youtubesidebar_settings, $YOUTUBESIDEBAR;
        $youtubesidebar_schedule_array = YOUTUBESIDEBAR::get_option_schedule_array();       
        $youtubesidebar_schedule_array['history']['lasteventaction'] = $run_event_type . ' Requested'; 
        ...
        switch ( $run_event_type) {
            ...
            case "postcreation":
                ...
                //youtubesidebar_event_return( 'post creation procedure complete' );
            ...
            case "postupdate":
                ...
                * 1st = Changed Records: search for records updated but not applied (post update function will update the youtubesidebar_applied or at least the sql function called will)
                * 2nd = Meta Key: youtubesidebar_outdated meta is searched, any meta value of 'yes' indicates posts needs to be updated 
                * 3rd = Changed Projected: project array includes a value ['updatecomplete'] which is set to true when no posts can be found with youtubesidebar_updated dates older than ['modified']. This allows an update to all posts. 
                */                                
                ...
                if( $record){
                    ...
                    $youtubesidebar_schedule_array['history']['lasteventaction'] = 'post with ID ' . $r['youtubesidebar_postid'] . ' was updated with a new record';        

                }
          
                ############################################################################
                #                                                                          #
                #             youtubesidebar_outdated META VALUE SET TO "yes" METHOD             #
                #                                                                          #
                ############################################################################         
                if(!$updating_carried_out){

                    // locate a post that have been marked as youtubesidebar_outdated
                    //$post = youtubesidebar_WP_SQL_get_posts_join_meta( 'youtubesidebar_outdated', 'yes', $limit = 1, $select = 'ID' );
                    if( $post){
                        ...
                        foreach( $post as $key => $p){
                            ...
                            $project_code = get_post_meta( $p->ID, 'youtubesidebar_project_code', true);
                            if(!$project_code){continue;}
                            
                            //youtubesidebar_post_updatepost( $p->ID,get_post_meta( $p->ID, 'youtubesidebar_record_id', true), $project_code);
             
                            //youtubesidebar_log_schedule( 'Post with ID '.$p->ID.' was updated because it was marked as outdated', 'post updated',1, $run_event_type, __LINE__, __FILE__, __FUNCTION__, 'medium' );
             
                            // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                            $youtubesidebar_schedule_array['history']['lasteventaction'] = 'post ' . $p->ID . ' was updated using youtubesidebar_outdated method';        
                        }
                    }                  
                }
                ...
                // against the youtubesidebar_updated meta value  
                if(!$updating_carried_out){
                    ...
                    */
                    ...
                    // run query for a post that has an older youtubesidebar_last_updated date than the projects modified date                        
                    $post = youtubesidebar_SQL_get_posts_oudated( $project_modified, $project_code, $limit = 1, $select = 'ID' );
                    ...
                    if( $post){
                        ...
                        foreach( $post as $key => $p){
                            ...
                            $record_id = get_post_meta( $p->ID, 'youtubesidebar_record_id', true);
                            if( $record_id && is_numeric( $record_id) ){
                                
                                // update the post, this function does not just use new records
                                //youtubesidebar_post_updatepost( $p->ID, $record_id, $project_code);
             
                                // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                                $youtubesidebar_schedule_array['history']['lasteventaction'] = 'post with ID ' . $p->ID . ' was updated because project configuration was changed';
                        ...
                        // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                        $youtubesidebar_schedule_array['history']['lasteventaction'] = 'schedule determine posts for project '.$outdated_project_code.' are all updated';
                ...
                youtubesidebar_event_return( 'data update procedure finished' ); 
    ...
    */
    public function pluginmediabutton_button( $context) {
        //append the icon
        $context = "<a class='button thickbox' title='YouTube Sidebar Column Replacement Tokens (CTRL + C then CTRL + V)'
        href='#TB_inline?width=400&inlineId=youtubesidebar_popup_container'>YouTube Sidebar</a>";
        return $context;
    }  
    ...
    */
    public function install_admin_settings() {
        require_once( YOUTUBESIDEBAR_ABSPATH . 'arrays/settings_array.php' );
        return $this->option( 'youtubesidebar_settings', 'update', $youtubesidebar_settings );# update creates record if it does not exist   
    } 
    ...
    */
    public function custom_post_types() { 
        ...
        if( isset( $youtubesidebar_settings['posttypes']['wtgflags']['status'] ) && $youtubesidebar_settings['posttypes']['wtgflags']['status'] === 'enabled' ) {    
            require( YOUTUBESIDEBAR_ABSPATH . 'posttypes/flags.php' );   
        }
        
        // has post "Edit Post" enhancements been setup for this package and activated in default settings?                                                       
        if( isset( $youtubesidebar_settings['posttypes']['posts']['status'] ) && $youtubesidebar_settings['posttypes']['posts']['status'] === 'enabled' ) {    
            require( YOUTUBESIDEBAR_ABSPATH . 'posttypes/posts.php' );   
        }
    }
    ...
    */
    public function request_made() {
        ...
        // YouTube Sidebar own special processing triggers
        if( isset( $_GET['youtubesidebaraction'] ) || isset( $_GET['nonceaction'] ) ){
            return true;
        }
    ...
    */
    public function newlog( $atts ){     
        global $youtubesidebar_settings, $wpdb, $youtubesidebar_currentversion;
        ...
        if( isset( $youtubesidebar_settings['globalsettings']['uselog'] ) && $youtubesidebar_settings['globalsettings']['uselog'] == 0){
            return false;
        }
        ...
        extract( shortcode_atts( array(  
            ...
            'version' => $youtubesidebar_currentversion, 
    ...
    */
    ...
    function conflict_prevention( $outputnoneactive = false ){
        ...
        $plugin_profiles[0]['title'] = __( 'Tweet My Post', 'youtubesidebar' );
        $plugin_profiles[0]['slug'] = 'tweet-my-post/tweet-my-post.php';
        $plugin_profiles[0]['author'] = 'ksg91';
        $plugin_profiles[0]['title_active'] = __( 'Tweet My Post Conflict', 'youtubesidebar' );
        $plugin_profiles[0]['message_active'] = __( 'Please deactivate Twitter plugins before performing mass post creation. This will avoid spamming Twitter and causing more processing while creating posts.', 'youtubesidebar' );
        $plugin_profiles[0]['message_inactive'] = __( 'If you activate this or any Twitter plugin please ensure the plugins options are not setup to perform mass tweets during post creation.', 'youtubesidebar' );
    ...
    */
    ...
    public function send_email( $recipients, $subject, $content, $content_type = 'html' ){     
        ...
        {
            add_filter( 'wp_mail_content_type', 'youtubesidebar_set_html_content_type' );
        }
        ...
        if( $content_type == 'html' )
        {    
            remove_filter( 'wp_mail_content_type', 'youtubesidebar_set_html_content_type' );  
        }   
    ...
    *  
    * @param mixed $page, registered page slug i.e. youtubesidebar_install which results in wp-admin/admin.php?page=youtubesidebar_install   
    ...
    */
    public function formend_standard( $buttontitle = 'Submit', $buttonid = 'notrequired' ){
            if( $buttonid == 'notrequired' ){
                $buttonid = 'youtubesidebar_notrequired'.rand(1000,1000000);# added during debug
            }else{
                $buttonid = $buttonid.'_formbutton';
            }?>

            <p class="submit">
                <input type="submit" name="youtubesidebar_wpsubmit" id="<?php echo $buttonid;?>" class="button button-primary" value="<?php echo $buttontitle;?>">
            </p>
    ...
    public function formstart_standard( $name, $id = 'none', $method = 'post', $class, $action = '', $enctype = '' ){
        ...
        echo '<form '.$class.' '.$enctype.' id="'.$id.'" method="'.$method.'" name="youtubesidebar_request_'.$name.'" action="'.$action.'">
        <input type="hidden" id="youtubesidebar_admin_action" name="youtubesidebar_admin_action" value="true">';
    } 
    ...
    */
    public function pageheader( $pagetitle, $layout ){
        global $current_user, $youtubesidebar_settings, $youtubesidebar_menu_array;
        ...
        <div id="youtubesidebar-page" class="wrap">
            ...
            // if not "YouTube Sidebar" set this title
            if( $pagetitle !== 'YouTube Sidebar' ) {
                $h2_title = 'YouTube Sidebar: ' . $pagetitle;    
            }

            // if build only has one page, shorten the title
            // this is to make plugin building a little quicker
            $pages = count( $youtubesidebar_menu_array );
            if( $pages == 1 ){
    ...
    */
    public function check_requirements( $display ){
        ...
        // php version
        if(defined(YOUTUBESIDEBAR_PHPVERSIONMINIMUM) ){
            if(YOUTUBESIDEBAR_PHPVERSIONMINIMUM > phpversion() ){
                ...
                if( $display == true){
                    ...
                    between PHP versions per WP installation or sometimes hosting allows customers to switch using their control panel.', 'youtubesidebar' ),YOUTUBESIDEBAR_PHPVERSIONMINIMUM)
                    , 'warning', 'Large', __( 'YouTube Sidebar Requires PHP ', 'youtubesidebar' ) . YOUTUBESIDEBAR_PHPVERSIONMINIMUM);                
                }
            }
        }
    ...
    /**
    * Wrapper, uses youtubesidebar_url_toadmin to create local admin url
    * 
    * @param mixed $page
    * @param mixed $values 
    */
    ...
    */
    ...
    public function get_installed_version() {
        return get_option( 'youtubesidebar_installedversion' );    
    }  
    ...
    */
    public static function get_option_schedule_array() {
        $youtubesidebar_schedule_array = get_option( 'youtubesidebar_schedule' );
        return maybe_unserialize( $youtubesidebar_schedule_array );    
    }
    ...
    */
    public function update_option_schedule_array( $schedule_array ){
        ...
        return update_option( 'youtubesidebar_schedule', $schedule_array_serialized);    
    }
    
    public function update_settings( $youtubesidebar_settings ){
        $admin_settings_array_serialized = maybe_serialize( $youtubesidebar_settings );
        return update_option( 'youtubesidebar_settings', $admin_settings_array_serialized);    
    }
    ...
    */
    public function event_return( $return_reason){
        $youtubesidebar_schedule_array = self::get_option_schedule_array();
        $youtubesidebar_schedule_array['history']['lastreturnreason'] = $return_reason;
        self::update_option_schedule_array( $youtubesidebar_schedule_array );   
    }  
    ...
    */
    ...
    public function create_localmedia_fromlocalimages( $file_url, $post_id ){           
        ...
        global $wpdb, $youtubesidebar_settings;
        ...
        if( isset( $youtubesidebar_settings['create_localmedia_fromlocalimages']['destinationdirectory'] ) ){   
            $artDir = $youtubesidebar_settings['create_localmedia_fromlocalimages']['destinationdirectory'];
        }else{
    ...
    */
    public function create_posts( $project_id, $total = 1 ){
        ...
        $autoblog = new YOUTUBESIDEBAR_InsertPost();
        $autoblog->settings = $youtubesidebar_settings;
    ...
    */
    public function update_posts( $project_id, $total = 1, $post_id = false, $atts = array() ){
        ...
        $autoblog = new YOUTUBESIDEBAR_UpdatePost();
        $autoblog->settings = $youtubesidebar_settings;
        $autoblog->maintable = $database_table;
    ...
    * @param mixed $level
    * 
    * @deprecated YOUTUBESIDEBAR_Categories class created
    */
    ...
    */
    public function postlist_addvideotosidebar( $limit ){
...
*/
class YOUTUBESIDEBAR_Log_Table extends WP_List_Table {
