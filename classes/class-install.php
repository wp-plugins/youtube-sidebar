...
class YOUTUBESIDEBAR_Install {
    ...
    */
    public function __construct() {
        ...
        // $this->DB = new YOUTUBESIDEBAR_DB(); commeted 14092014
        $this->DB = YOUTUBESIDEBAR::load_class( 'YOUTUBESIDEBAR_DB', 'class-wpdb.php', 'classes' );
        $this->PHP = new YOUTUBESIDEBAR_PHP();
                
        // on activation run install_plugin() method which then runs more methods i.e. create_tables();
        register_activation_hook( YOUTUBESIDEBAR_ABSPATH . 'youtubesidebar.php', array( $this, 'install_plugin' ) ); 

        // on deactivation run disabled_plugin() - not a full uninstall
        register_deactivation_hook( YOUTUBESIDEBAR_ABSPATH . 'youtubesidebar.php',  array( $this, 'deactivate_plugin' ) );
    ...
    */
    function create_tables() {       
        global $charset_collate, $wpdb, $YOUTUBESIDEBAR;
    ...
    */
    public function reinstalldatabasetables() {
        ...
        require_once( YOUTUBESIDEBAR_ABSPATH . 'arrays/tableschema_array.php' );
        
        if(is_array( $youtubesidebar_tables_array ) ){
            foreach( $youtubesidebar_tables_array['tables'] as $key => $table){
    ...
    function install_options() {
        ...
        update_option( 'youtubesidebar_installedversion', YOUTUBESIDEBAR::version );# will only be updated when user prompted to upgrade rather than activation
        update_option( 'youtubesidebar_installeddate',time() );# update the installed date, this includes the installed date of new versions
        
        // schedule settings
        require( YOUTUBESIDEBAR_ABSPATH . 'arrays/schedule_array.php' );        
        add_option( 'youtubesidebar_schedule', serialize( $youtubesidebar_schedule_array ) );

        // notifications array (persistent notice feature)
        add_option( 'youtubesidebar_notifications', serialize( array() ) ); 
    }
    ...
    function install_plugin() {              
        ...
        update_option( 'youtubesidebar_is_installed', true );
    } 
