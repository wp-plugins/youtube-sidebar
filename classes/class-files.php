...
class YOUTUBESIDEBAR_Files { 
    
    public function __construct() {
        $this->PHP = YOUTUBESIDEBAR::load_class( 'YOUTUBESIDEBAR_PHP', 'class-phplibrary.php', 'classes' ); # php library by Ryan R. Bayne  
    }
    ...
    */
    public function upload_error( $file_error ) {
        $upload_error_strings = array( false,
            __( "The uploaded file exceeds the <code>upload_max_filesize</code> directive in <code>php.ini</code>.", 'youtubesidebar' ),
            __( "The uploaded file exceeds the <em>MAX_FILE_SIZE</em> directive that was specified in the HTML form.", 'youtubesidebar' ),
            __( "The uploaded file was only partially uploaded.", 'youtubesidebar' ),
            __( "No file was uploaded.", 'youtubesidebar' ),
            __( "Missing a temporary folder.", 'youtubesidebar' ),
            __( "Failed to write file to disk.", 'youtubesidebar' ) );  
    ...
    */
    public function singlefile_uploader( $file_array, $uploads = false, $test_size = true, $test_type = false, $overwrite = false ) {
        $result_array = array( 'outcome' => false,
                               'error' => false, 
                               'message' => __( 'The file was uploaded.', 'youtubesidebar' ) );
        ...
        if ( $test_size && !( $file_array['size'] > 0 ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Your file is empty. Empty files are not permitted in this operation.', 'youtubesidebar' );
            return $result_array;            
        }
        ...
        if (! @ is_uploaded_file( $file_array['tmp_name'] ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Sorry your file upload failed. A temporary file could not be created on the server. This may be a temporary problem, please try again.', 'youtubesidebar' );
            return $result_array;                    
        }
        ...
        if ( $test_type ) {
            ...
            if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file does not meet security requirements and has not been uploaded.', 'youtubesidebar' );
                return $result_array;                
            }
        ...
        if ( false === @ move_uploaded_file( $file_array['tmp_name'], $uploads['path'] . $filename ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file could not be moved into the set directory. If a file with the same name exists in that directory an overwrite may not be possible.', 'youtubesidebar' );
                return $result_array; 
        }    
    ...
    */
    public function file_from_url( $url, $uploads = array(), $overwrite = false, $allow = false ) {
        ...
        if( !$url_is_valid ) {
            $result['message'] = __( 'URL is invalid', 'youtubesidebar' );
            return false;    
        }
        ...
        if ( false !== $uploads['error'] ) {
            $result['message'] = __( 'WordPress wp_upload_dir() returned an error.', 'youtubesidebar' );
        ...
        if( $write_result === false ) {
            ...
            if( file_exists( $uploads['path'] . $file['originalName'] ) ) {
                $result['message'] = __( 'The filename already exists and the file cannot be overwritten.', 'youtubesidebar' );
                return $result;                 
            }
            
        } elseif( $write_result === false && $rename !== true ) {
            $result['failurereason'] = __( 'File already exists and renaming is not allowed.', 'youtubesidebar' );
            return $result;                
        }
        ...
        $message = __( 'The file was transferred.', 'youtubesidebar' ); 
        
        if( $overwrite_applied ) {
            $message .= ' ' . __( 'An existing file was overwritten as requested.', 'youtubesidebar' );
        }
        
        if( $filerename_applied ) {
            $message .= ' ' . __( 'The files name had to be changed as it already exists.', 'youtubesidebar' );    
        }
    ...
    */
    public function compare_csv_files( $filepath_one, $filepath_two ) {
        ...
        can switch to the newer file if required.', 'youtubesidebar' );      
