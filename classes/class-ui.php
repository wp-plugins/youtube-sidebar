...
class YOUTUBESIDEBAR_UI extends YOUTUBESIDEBAR {     
    ...
    public function __construct() {
        ...
        $this->WPCore = self::load_class( 'YOUTUBESIDEBAR_WPCore', 'class-wpcore.php', 'classes' );  
    }  
    ...
    * 
    */
    public function GUI_tablestart($class = false){
    ...
    public function echoselected( $value, $argument){if( $value == $argument ){echo 'selected="selected"';}}
         
    /**
    * Called by main YOUTUBESIDEBAR() class using proper hook
    * 
    ...
    */
    public function add_dashboard_widgets() {
        ...
        wp_add_dashboard_widget(
             ...
             __( 'Test Widget', 'youtubesidebar' ),// Title.
        ...
        foreach( $menu_array as $key => $section_array ) {
            ...
            if( !isset( $youtubesidebar_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] ) ) {       
                continue;    
            } elseif( $youtubesidebar_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] !== 'enabled' ) {    
                continue;    
            }
            ...
            if( !isset( $youtubesidebar_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {      
                continue;
            } elseif( !current_user_can( $youtubesidebar_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {    
            ...
            $view_object = YOUTUBESIDEBAR::load_class( 'YOUTUBESIDEBAR_' . ucfirst( $section_array['name'] ) . '_View', $section_array['name'] . '.php', 'views' );
    ...
    */
    ...
    /**
    * Displays a table of youtubesidebar_option records with ability to view their value or delete them
    ...
    */
    public function list_optionrecordtrace( $form = false, $size = 'Small', $optiontype = 'all' ){ 
        // first get all records that begin with youtubesidebar_
        $youtubesidebarrecords_result = $this->DB->options_beginning_with( 'youtubesidebar_' );
        $counter = 1;?>
            
            <script language="JavaScript">
            function youtubesidebar_deleteoptions_checkboxtoggle(source) {
              checkboxes = document.getElementsByName( 'youtubesidebar_deleteoptions_array[]' );
        ...
        if( $form){
            echo '<input type="checkbox" onClick="youtubesidebar_deleteoptions_checkboxtoggle(this)" /> '. __( 'Select All Options', 'youtubesidebar' ) .'<br/>';
        }
        ...
        foreach( $youtubesidebarrecords_result as $key => $option ){
            ...
            if( $form){
                $html = '<input type="checkbox" name="youtubesidebar_deleteoptions_array[]" value="'.$option.'" />';
            }
    ...
    public function screens_menuoptions( $current){
        ...
        foreach( $youtubesidebar_menu_array as $page_slug => $page_array ){ 
            foreach( $youtubesidebar_menu_array[$page_slug]['tabs'] as $whichvalue => $screen_array ){
                $selected = '';
    ...
    public function page_menuoptions( $current, $return = false ){
        ...
        foreach( $youtubesidebar_menu_array as $page_slug => $page_array ){ 
            ...
            if( $return){
                return '<option value="'.$page_slug.'" '.$selected.'>'.$youtubesidebar_menu_array[$page_slug]['viewtitle'].'</option>';
            }else{ 
                echo '<option value="'.$page_slug.'" '.$selected.'>'.$youtubesidebar_menu_array[$page_slug]['viewtitle'].'</option>';
            }
        }    
    }
    ...
    */    
    public function status_box( $title, $intro, $progress_array = array() ){
        ...
        echo '
        <div class="youtubesidebar_status_box_container">
    ...
    */
    public function option_switch( $title, $name, $id, $current_value = 'nocurrent123', $onlabel = 'Enabled', $offlabel = 'Disabled', $default = 'enabled' ){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $title, 'youtubesidebar' ); ?></th>
            <td>
    ...
    * @deprecated use class-forms.php      
    */
    public function option_text( $title, $name, $id, $current_value = '', $readonly = false, $class = 'youtubesidebar_inputtext', $append_text = '', $left_field = false, $right_field_content = '', $required = false, $validation = false ){
    ...
    */
    public function option_text_simple( $title, $nameandid, $current_value = '', $required = false, $validation = false ){
        $this->option_text( $title, $nameandid, $nameandid, $current_value, false, 'youtubesidebar_inputtext', '', false, '', $required, $validation );
    }
    ...
    */
    public function metabox_icons( $form_id, $trash_icon = false ){
        ...
        if( YOUTUBESIDEBAR::is_dashboard() ) {
            return false;    
        }
        ...
        // load help array
        $YOUTUBESIDEBAR_Help = YOUTUBESIDEBAR::load_class( 'YOUTUBESIDEBAR_Help', 'class-help.php', 'classes' );
                    
        $help_array = $YOUTUBESIDEBAR_Help->get_help_array();

        $total_icons = 0; 

        $page_name = YOUTUBESIDEBAR::get_admin_page_name();
                
        // display video icon with link to a YouTube video
        if( isset( $help_array[ $page_name ][ YOUTUBESIDEBAR_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] ) ){
            self::panel_video_icon( $help_array[ $page_name ][ YOUTUBESIDEBAR_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] );
            ++$total_icons;        
        } 

        // do we have all required values to build help content and display form_information_icon()
        if( isset( $help_array[ $page_name ][ YOUTUBESIDEBAR_VIEWNAME ][ 'forms' ][ $form_id ][ 'formtitle' ] )
            && isset( $help_array[ $page_name ][ YOUTUBESIDEBAR_VIEWNAME ][ 'forms' ][ $form_id ][ 'formabout' ] ) ){  
                           
            // display the icon that will show information about the box when clicked
            self::box_information_icon( $page_name, YOUTUBESIDEBAR_VIEWNAME, $form_id );
        }
    ...
    public function panel_trash_icon( $form_name ){?>
        <a href="<?php echo $form_name;?>"><img src="<?php echo YOUTUBESIDEBAR_IMAGES_URL . 'trash-icon.gif';?>" alt="<?php _e( 'Reset form' );?>" title="<?php _e( 'Reset form' );?>"></a><?php 
    }
    
    public function panel_video_icon( $youtubeid ){
        if( $youtubeid){
            echo '<a href="https://www.youtube.com/watch?v=' . $youtubeid . '" target="_blank"><img src="' . YOUTUBESIDEBAR_IMAGES_URL . 'video-icon.gif" alt="' . __( 'View video on YouTube' ) . '" title="' . __( 'View video on YouTube' ) . '"></a>';
        }
    }
    ...
    */
    public function helpicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View help for this on the plugins website.' ) .'" target="_blank"><img src="'. YOUTUBESIDEBAR_IMAGES_URL . 'help-icon.png" alt="'. __( 'View help for this on the plugins website' ) .'"></a>';
    }    
    public function videoicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View a video about this feature.' ) .'" target="_blank"><img src="'. YOUTUBESIDEBAR_IMAGES_URL . 'video-icon.png" alt="'. __( 'Video icon, click to open a video' ) .'"></a>';
    }     
    public function trashicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Delete or hide the current feature.' ) .'" target="_blank"><img src="'. YOUTUBESIDEBAR_IMAGES_URL . 'trash-icon.png" alt="'. __( 'Trash icon, click to delete or hide the current item' ) .'"></a>';
    }    
    public function discussicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Discuss this feature on the WebTechGlobal forum.' ) .'" target="_blank"><img src="'. YOUTUBESIDEBAR_IMAGES_URL . 'chat-icon.png" alt="'. __( 'Chat icon which you can click to discuss this feature on the WebTechGlobal forum.' ) .'"></a>';
    }  
    ...
    */
    public function postbox_content_header( $form_title, $form_id, $introduction = false, $trash_icon = false, $uploader = false ) {?>
        <p>
            ...
            <?php if( $introduction !== false && !empty( $introduction) ){echo "<p class=\"youtubesidebar_boxes_introtext\">". $introduction ."</p>";}?>
    ...
    */          
    public function box_information_icon( $page_name, $view_name, $form_id ) {
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'You are welcome to visit the', 'youtubesidebar' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the YouTube Sidebar website and read more about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';
            $all_help_content .= __( 'YouTube Sidebar Website', 'youtubesidebar' ) . '</a> ' . __( 'to read more about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'There is a', 'youtubesidebar' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] . '"';
            $all_help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= __( 'YouTube Video', 'youtubesidebar' ) . '</a> ' . __( 'about', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'We invite you to take discuss', 'youtubesidebar' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'youtubesidebar' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'youtubesidebar' );           
            $all_help_content .= '.</p>';
        }        
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] ) ){
            foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] as $option_id => $option_array ){
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $all_help_content .= ' title="' . __( 'Read More about', 'youtubesidebar' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Read More', 'youtubesidebar' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $all_help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $all_help_content .= ' title="' . __( 'Watch a video about', 'youtubesidebar' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Video', 'youtubesidebar' ) . '</a>';      
                }
        ...
        $all_help_content .= __( 'Help for ', 'youtubesidebar' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '">';
        $all_help_content .= '<img src="' . YOUTUBESIDEBAR_IMAGES_URL . 'info-icon.gif" alt="';
        $all_help_content .= __( 'Click icon to view help for ', 'youtubesidebar' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"></a>';
      
        echo $all_help_content;
    }
                                
    public function get_notice_array() {
        $a = get_option( 'youtubesidebar_notifications' );
       
        $youtubesidebar_notice_array = maybe_unserialize( $a);
        if(!is_array( $youtubesidebar_notice_array ) ){
            return array();    
        }
        
        // delete some expired admin notices
        if( isset( $youtubesidebar_notice_array['admin'] ) )
        {
            foreach( $youtubesidebar_notice_array['admin'] as $key => $notice){
                ...
                if( isset( $notice['created'] ) )
                {
                    ...
                    if( $projected < time() )
                    {
                        unset( $youtubesidebar_notice_array['admin'][$key] );    
                    }                                               
                }
                else
                {       
        ...
        if( isset( $youtubesidebar_notice_array['users'] ) ){
 
            foreach( $youtubesidebar_notice_array['users'] as $owner_id => $owners_notices){
                ...
                foreach( $owners_notices as $key => $notice)
                {
                    if( isset( $notice['created'] ) )
                    {
                        ...
                        if( $projected < time() )
                        {
                            unset( $youtubesidebar_notice_array['users'][$key] );    
                        }                                               
                    }
                    else
                    {
        ...
        return $youtubesidebar_notice_array;    
    }
    
    public function update_notice_array() {
        global $youtubesidebar_notice_array;      
        return update_option( 'youtubesidebar_notifications',maybe_serialize( $youtubesidebar_notice_array ) );    
    }    
    ...
    */
    public function notice_return( $type, $size, $title = false, $message = 'no message has been set', $helpurl = false, $forcewtgstyle = false ){
        ...
        if( $forcewtgstyle === false ) {
            if( isset( $youtubesidebar_settings['noticesettings']['wpcorestyle'] ) && $youtubesidebar_settings['noticesettings']['wpcorestyle'] == 'enabled' ) {
    ...
    */
    public function create_notice( $message, $type = 'info', $size = 'Small', $title = false, $sensitive = false, $helpurl = false ){
        global $youtubesidebar_notice_array, $current_user;
        ...
        if( $sensitive === false ) {   
            ...
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['sensitive'] = false;
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['message'] = $message;
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['type'] = $type;
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['size'] = $size;
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['title'] = $title; 
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['created'] = time();
            $youtubesidebar_notice_array['notices'][$current_user->ID][$key]['helpurl'] = $helpurl;
            ...
            $youtubesidebar_notice_array['users'][$current_user->ID][$key]['sensitive'] = false;
        ...
        return $this->update_notice_array( $youtubesidebar_notice_array );
    }
    ...
    */
    public function create_prompt() {
        ## $youtubesidebar_notice_array['prompts'][$owner][$key]['message'] = $message;    
    }
    ...
    */
    public function create_message() {
        ## $youtubesidebar_notice_array['messages'][$owner][$key]['message'] = $message;    
    }
    ...
    public function display_users_notices() {
        global $youtubesidebar_notice_array, $current_user, $youtubesidebar_settings;
        
        $youtubesidebar_notice_array = $this->get_notice_array();
        
        if(!isset( $youtubesidebar_notice_array['notices'][$current_user->ID] ) ){return;}
 
        foreach( $youtubesidebar_notice_array['notices'][$current_user->ID] as $key => $owners_notices){
            ...
            unset( $youtubesidebar_notice_array['notices'][$current_user->ID] );
        }
                                       
        $this->update_notice_array( $youtubesidebar_notice_array );            
    }
    ...
    * 
    * @deprecated use youtubesidebar_n_postresult as of 13th February 2013
    */
    ...
    /**
    * New Notice Function (replacing YOUTUBESIDEBAR_Notice::notice_depreciated() )
    ...
    */
    public function n_depreciated( $title, $mes, $style, $size, $atts = array() ){
        ...
        // this allows the notice to be printed within content where the function is called rather than within the $youtubesidebar_notice_array loop
        ...
        $youtubesidebar_notice_array = $this->persistentnotifications_array();
        
        // set next array key value
        $next_key = 0;

        // determine next array key
        if( isset( $youtubesidebar_notice_array['notifications'] ) ){    
            $next_key = YOUTUBESIDEBAR::get_array_nextkey( $youtubesidebar_notice_array['notifications'] );
        }    
        ...
        $youtubesidebar_notice_array['notifications'][$next_key]['message'] = $mes;
        $youtubesidebar_notice_array['notifications'][$next_key]['type'] = $style;
        $youtubesidebar_notice_array['notifications'][$next_key]['size'] = $size;
        $youtubesidebar_notice_array['notifications'][$next_key]['title'] = $title;
        $youtubesidebar_notice_array['notifications'][$next_key]['helpurl'] = $url; 
        $youtubesidebar_notice_array['notifications'][$next_key]['output'] = $output;
        $youtubesidebar_notice_array['notifications'][$next_key]['audience'] = $audience;
        $youtubesidebar_notice_array['notifications'][$next_key]['user_mes'] = $user_mes;
        $youtubesidebar_notice_array['notifications'][$next_key]['side'] = $side;
        $youtubesidebar_notice_array['notifications'][$next_key]['clickable'] = $clickable;
                          
        $this->update_persistentnotifications_array( $youtubesidebar_notice_array );
    } 
    ...
    */
    public function update_persistentnotifications_array( $notifications_array ){
        return update_option( 'youtubesidebar_notifications',maybe_serialize( $notifications_array ) );    
    }
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'www.webtechglobal.co.uk', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        ...
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $helpurl != false && $helpurl != '' && $helpurl != 'http://www.youtubesidebar.com/support' ){$clickable = false;}
            ...
            if( $output_type == 'return' || $output_type == 'public' ){   
                ...
                // add new message to the notifications array
                $youtubesidebar_notice_array['notifications'][$next_key]['message'] = $message;
                $youtubesidebar_notice_array['notifications'][$next_key]['type'] = $type;
                $youtubesidebar_notice_array['notifications'][$next_key]['size'] = $size;
                $youtubesidebar_notice_array['notifications'][$next_key]['title'] = $title;
                $youtubesidebar_notice_array['notifications'][$next_key]['helpurl'] = $helpurl; 
                $youtubesidebar_notice_array['notifications'][$next_key]['clickable'] = $clickable; 
    ...
    /**                                              
    * Outputs the contents of $youtubesidebar_notice_array, used in YOUTUBESIDEBAR::pageheader().
    * Will hold new and none persistent notifications. May also hold persistent. 
    */
    public function output_depreciated() {
        $youtubesidebar_notice_array = self::persistentnotifications_array();    
        if( isset( $youtubesidebar_notice_array['notifications'] ) ){
                                                        
            foreach( $youtubesidebar_notice_array['notifications'] as $key => $notice){
                ...
                if(!isset( $notice['persistent'] ) || $notice['persistent'] != false ){
                    ...
                    unset( $youtubesidebar_notice_array['notifications'][$key] );
                }
            }
    ...
    */
    ...
    public function add_admin_footer_text() {
        // show admin footer message (only on pages of YouTube Sidebar)
        add_filter( 'admin_footer_text', array( $this, '_admin_footer_text' ) );
    }
    
    /**
     * Add a YouTube Sidebar "Thank You" message to the admin footer content
     *
     * @since 3.0.0
     *
    ...
    public function _admin_footer_text( $content ) {
        global $c2p_is_free;
        $content .= ' &bull; ' . __( 'Thank you for using <a href="http://webtechglobal.co.uk/youtubesidebar/">YouTube Sidebar</a>.', 'youtubesidebar' );
        // FREE EDITIONS ONLY $content .= ' ' . sprintf( __( 'Support the plugin with your <a href="%s">donation</a>!', 'youtubesidebar' ), 'http://webtechglobal.co.uk/donate/' );
        return $content;
    } 
    ...
    * @param mixed $values - begin with & followed by values
    */
    public function linkaction( $page, $action, $title = 'YouTube Sidebar admin link', $text = 'Click Here', $values = '' ){
        return '<a href="'. wp_nonce_url( admin_url() . 'admin.php?page=' . $page . '&youtubesidebaraction=' . $action  . $values, $action ) . '" title="' . $title . '" class="button c2pbutton">' . $text . '</a>';
    }
    ...
    */
    public function register_input_validation( $input_title, $input_name, $input_id, $input_validation ) {
        ...
        $form_val_arr = get_option( 'youtubesidebar_formvalidation' );
        ...
        update_option( 'youtubesidebar_formvalidation', $form_val_arr );
    }
    ...
    */
    public function metabox_nocurrentproject() {
        ...
        the one that was active.', 'youtubesidebar' );
        echo '</p>';
    }
